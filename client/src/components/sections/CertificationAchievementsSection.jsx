...
import CertificationAchievementCard from "../certification-achievements/CertificationAchievementCard";
...
const defaultSectionContent = {
  ...
  description:
    "Explore professional certifications, licenses, awards and achievements that validate practical skills, learning and recognition.",
  ctaButton: {
...
function CertificationAchievementsSection() {
  ...
  const achievements = Array.isArray(achievementRecords)
    ? achievementRecords
    : [];
      ...
      <Container>
        ...
        <p aria-live="polite" className="sr-only">
          {isLoading
            ...
            : `${achievements.length} Certifications and Achievements loaded.`}
        </p>
        ...
        {!isLoading && !error && achievements.length === 0 && (
          <div className="mt-10 rounded-3xl border border-slate-200 bg-white px-6 py-12 text-center shadow-sm">
            ...
            <p className="mt-6 text-lg font-bold text-slate-950">
              No public Certifications or Achievements available
            </p>

            <p className="mx-auto mt-2 max-w-xl text-sm leading-6 text-slate-500">
              Published certifications, licenses, awards and achievements will
              appear here.
            </p>
          </div>
        )}
          ...
          <div className="mt-10 grid gap-6 md:grid-cols-2 xl:grid-cols-4">
            {previewAchievements.map((achievement) => (
              <CertificationAchievementCard
                key={achievement._id || achievement.id || achievement.slug}
        ...
        {previewAchievements.length > 0 && shouldShowCta && (
          <div className="mt-8 flex flex-col gap-4 rounded-2xl border border-brand-100 bg-brand-50 px-5 py-5 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <p className="font-bold text-slate-950">
                Explore all professional credentials and recognition
              </p>

              <p className="mt-1 text-sm leading-6 text-slate-600">
                View every published certification, license, award and
                achievement.
              </p>
            </div>
