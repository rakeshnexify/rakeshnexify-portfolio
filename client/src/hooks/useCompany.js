import { useEffect, useState } from "react";

import { fetchPublicCompanyBySlug } from "../services/companiesApi";

function createInitialRequestState(slug) {
  const hasSlug = Boolean(slug);

  return {
    slug,
    company: null,
    isLoading: hasSlug,
    error: hasSlug ? "" : "Company slug is required.",
    status: hasSlug ? null : 400,
  };
}

function useCompany(slug) {
  const normalisedSlug = String(slug || "")
    .trim()
    .toLowerCase();

  const [requestState, setRequestState] = useState(() =>
    createInitialRequestState(normalisedSlug),
  );

  const [refreshKey, setRefreshKey] = useState(0);

  useEffect(() => {
    if (!normalisedSlug) {
      return undefined;
    }

    const controller = new AbortController();

    async function loadCompany() {
      try {
        const company = await fetchPublicCompanyBySlug(normalisedSlug, {
          signal: controller.signal,
        });

        setRequestState({
          slug: normalisedSlug,
          company,
          isLoading: false,
          error: "",
          status: 200,
        });
      } catch (requestError) {
        if (requestError?.name === "AbortError") {
          return;
        }

        console.error("Public company loading failed:", requestError);

        setRequestState({
          slug: normalisedSlug,
          company: null,
          isLoading: false,

          error:
            requestError instanceof Error
              ? requestError.message
              : "Company could not be loaded.",

          status: requestError?.status || null,
        });
      }
    }

    loadCompany();

    return () => {
      controller.abort();
    };
  }, [normalisedSlug, refreshKey]);

  const activeState =
    requestState.slug === normalisedSlug
      ? requestState
      : createInitialRequestState(normalisedSlug);

  function refreshCompany() {
    if (!normalisedSlug) {
      return;
    }

    setRequestState(createInitialRequestState(normalisedSlug));

    setRefreshKey((currentKey) => currentKey + 1);
  }

  return {
    company: activeState.company,
    isLoading: activeState.isLoading,
    error: activeState.error,
    status: activeState.status,
    refreshCompany,
  };
}

export default useCompany;
